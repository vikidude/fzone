    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: 'black' }}>
            <Text style={{ textAlign: 'center', color: 'white',marginTop: height *0.01, fontSize: width *0.06, fontWeight:'bold' }}>Subscriptions</Text>
                ...
                <FlatList 
                    data = {plans}
                    contentContainerStyle = {{backgroundColor: 'white', width: width * 0.9, margin: width * 0.05, 
                        ...
                        <>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between',alignItems:'flex-end' }}>
                                <View style={{flexDirection:'row',alignItems:'center'}}>
                                    ...
                                    <View style={{ width: width * 0.05, height: width * 0.05, borderRadius: (width * 0.05) / 2, borderWidth: 1,
                                        borderColor: 'black', backgroundColor: item.selected?'#c94f9a':'white' }} />
                                    {item.selected === true &&<Entypo name='check' color='white' 
                                    style={{ position: 'absolute', top: height * 0.0045, 
                                    left: width * 0.008 }} />}
                                    </Pressable>
                                    <Text style={{ fontSize: 18,marginLeft: width * 0.025 }}>{item.term}</Text>
                                </View>
                                <View style={{ flexDirection: 'column',alignItems:'center'}}>
                                    <View style={{flexDirection:'row'}}>
                                        <Text style={{fontSize: width * 0.05, textDecorationLine:'line-through', textDecorationColor:'grey',color:'grey'}}>{item.realPrice}</Text>
                                        <Text style={{fontSize: width * 0.05,fontWeight:'bold'}}>{' '}{typeof(item.offerPrice) === 'number'? '$': ''} {item.offerPrice}</Text>
                                    </View>
                                    {item.perMonth !== '' &&<Text style={{fontSize: width * 0.04}}>{item.perMonth}/mon</Text>}
                                </View>
                            </View>
                            {item.bottomText !== ''? (<Text style={{fontSize: 14, marginVertical: height * 0.03, color:'grey'}}>{item.bottomText} | <Text style={{ textDecorationLine: 'underline' }}>+1 offers</Text></Text>)
                            :
